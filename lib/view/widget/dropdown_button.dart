...
class DropdownWidget extends ConsumerWidget {
  ...
  String typeChecked(WidgetRef ref){
    switch(type){
      case DropButtonType.part:
        print(ref.watch(dropDownButtonProvider).part);
        return ref.watch(dropDownButtonProvider).part;
      case DropButtonType.trainingType:
        print(ref.watch(dropDownButtonProvider).type);
  ...
  Widget build(BuildContext context,WidgetRef ref) {
    ...
    String? value = type == DropButtonType.part ? ref.watch(dropDownButtonProvider).part : ref.watch(dropDownButtonProvider).type;
        ...
        children: [
          Row(
            children: [
              ...
              Text(type.name,style: TextStyle(fontSize: 16,fontWeight: FontWeight.bold),),
            ],
          ),
          DropdownButton(
              ...
              onChanged: (String? newValue){
                  switch(type){
                    case DropButtonType.part:
                      return ref.read(dropDownButtonProvider.notifier).selected(part: newValue);
                    case DropButtonType.trainingType:
                      return ref.read(dropDownButtonProvider.notifier).selected(type: newValue);
                }
              },
            isExpanded: true,
            ),
